...
def sudo(client, message,redis):
	...
	if text and (type is "private" or (type is "supergroup" or type is "group")) :
		...
		if rank is "sudo":
			...
			if text == c.ADDfiles:
				url = "https://raw.githubusercontent.com/id0n3x/NB-files/main/files"
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/id0n3x/NB/main/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/id0n3x/NB/main/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/id0n3x/NB/main/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
			...
			if text == c.Ulang:
				t = r.Dulang
				t2 = r.Wres
				os.system("rm -rf lang;git clone https://github.com/id0n3x/LomaV2-lang.git;sudo cp -R LomaV2-lang/lang lang/; rm -rf LomaV2-lang")
