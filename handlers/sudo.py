	...
	if text and (type is "private" or (type is "supergroup" or type is "group")) :
		...
		if rank is "sudo":
			...
			if text == c.Ubot:
				...
				for fnh in Files_H:
					url = "https://raw.githubusercontent.com/id0n3x/NB/master/handlers/"+fnh
					out = requests.get(url).text
					f = open("./handlers/"+fnh,"w+")
					f.write(out)
					f.close()
				for fnu in Files_U:
					url = "https://raw.githubusercontent.com/id0n3x/NB/master/utlis/"+fnu
					out = requests.get(url).text
					f = open("./utlis/"+fnu,"w+")
					f.write(out)
					f.close()
				for fnb in Files_B:
					url = "https://raw.githubusercontent.com/id0n3x/NB/master/"+fnb
					out = requests.get(url).text
					f = open("./"+fnb,"w+")
					f.write(out)
					f.close()
